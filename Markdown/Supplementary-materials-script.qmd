...
occurrences of [start]{.smallcaps} across the stimuli. We also note the
...
design. Each participant listened to two 'blocks' of 19 stimuli. In each
block, each speaker would be heard once. A simple random sampling of
stimuli for each participant would have resulted in an uneven
distribution of rating counts for each pair, with some pairs likely to
never be rated at all. We instead used a semi-random sampling procedure
that distributed the possible combinations of stimuli pairs as evenly as
possible across the stimuli subsets participants heard. In effect, for
approximately every 18 participants all 703 stimuli pairs are listened
to once. Participant drop-out meant that, in practice, the overall
distribution of the possible pairwise combinations was not as even as
intended.
...
@fig-pair-summary and @tbl-pairwise-stats provide an overview of the
distribution of ratings. On average, each pair was rated 7 times.
...
However, as @fig-mds-scree illustrates, stress will *always* decrease as
the number of dimensions increases (i.e., 4 dimensions will always have
a lower stress than 3, which will always have a lower stress than 2). As
such, we do not consider relying on a single stress value to be best
practice [following @RN620].
...
@fig-bubble instead displays a "bubble" plot of the stress-per-point
(SPP) of the individual objects (i.e., speakers) in the data. The larger
the 'bubble' for each speaker, the more they contribute to the misfit of
the MDS (i.e., the higher their SPP). All speakers have an SPP below 5%
(min = 1.76, max = 4.6). Speakers 2, 37 and 25 have the highest SPP
values (\>4), but are all within 3 standard deviations of the mean SPP
value (4.83). Given they also do not inhibit the interpretability of the
perceptual similarity space, we proceed with the reported MDS analysis.
...
In this section, we apply PERMANOVA to test whether the proposed groups
in @fig-MDS-output-combined-analysis are statistically distinct. First,
we extract the main group each speaker belongs to in the next code
chunk.
...
We now apply standard and pairwise PERMANOVA in the next chunk. Standard
PERMANOVA tests the null hypothesis that the centroids (mean middle
point) and dispersion of our five speaker groups are equivalent. The
result indicates that we can reject this null hypothesis, and that our
speaker groups are not all equivalent. This application of PERMANOVA
does not, however, tell us which groups are distinct from each other. TO
investigate this, we apply pairwise PERMANOVA using a wrapper for the
`adonis` function.

The pairwise PERMANOVA indicates not all groups are statistically
distinct. Instead, opposing groups on either side of the fast leaders at
the centre of the space are statistically distinct from each other.
Specifically, fast/high pitch laggers are distinct from slow/low pitch
laggers, and slow/low pitch leaders are distinct from both high pitch
speakers and fast/high pitch laggers. Finally, slow/low leaders and
laggers are significantly different from each other.
...
but the dispersion of each group is different. As such, it is worth
checking if the dispersion of our five groups are markedly different. We
can again use the `vegan` package to identify the dispersion of each
group, and test the null hypothesis that they are equivalent. The
results indicate the groups have comparable levels of dispersion,
meaning the results of the PERMANOVA are likely driven by differences in
group centroids.
