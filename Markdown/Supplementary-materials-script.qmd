...
PW_ratings_ID %>% 
  ...
  ungroup() %>% 
ggplot(aes(y =PairCount, x="Unordered")) +geom_jitter(alpha=0.2)+geom_boxplot(notch = T)+
  labs(x="Pairs")+
  PW_ratings_ID %>% 
  group_by(pair_id_ordered) %>% 
  summarise(PairCount=n()) %>% 
  ungroup() %>% 
ggplot(aes(y =PairCount,x="Ordered")) +geom_jitter(alpha=0.2) +geom_boxplot(notch = T)+
  labs(x="Pairs")
